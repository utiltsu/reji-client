...
8. **Check shadcn before building a UI primitive.** `dialog`, `sheet`, `command`, `popover`,
   ...
   All form fields use shadcn/ui `Field` composition. Use `Field` + `FieldLabel` + a shadcn
   control such as `Input`, `Textarea`, `Select`, `Checkbox`, or `RadioGroup`, with
   `FieldDescription` and `FieldError` when needed. Feature components must not render raw
   `<input>`, `<textarea>`, `<select>`, checkbox, or radio controls directly.
